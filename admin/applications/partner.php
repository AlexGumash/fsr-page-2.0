...
$query = "SELECT * FROM `user-partner-info` JOIN users ON `user-partner-info`.userid = users.id";
    ...
    while ($application = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
      ...
      ?>
        <tr class="" id="application<?php echo $i ?>">
          ...
          <td class="">
            <div onclick="acceptDicline(<?php echo $i?>, 'partner', 'accepted', <?php echo $application['userid']?>);">
              <span>Accept</span>
            </div>
            <div onclick="acceptDicline(<?php echo $i?>, 'partner', 'declined', <?php echo $application['userid']?>);">
              <span>Decline</span>
            </div>
          </td>
        </tr>
