...
$query = "SELECT * FROM `user-guest-info` JOIN users ON `user-guest-info`.userid = users.id";
    ...
    <?php
        ...
        while ($application = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
          ...
          <tr class="" id="application<?php echo $i ?>">
            ...
            <td class="">
              <div onclick="acceptDicline(<?php echo $i?>, 'guest', 'accepted', <?php echo $application['userid']?>);">
                <span>Accept</span>
              </div>
              <div onclick="acceptDicline(<?php echo $i?>, 'guest', 'declined', <?php echo $application['userid']?>);">
                <span>Decline</span>
              </div>
            </td>
          </tr>
