...
$query = "SELECT * FROM `event-docs` JOIN `event-docs-approval` ON `event-docs`.id = `event-docs-approval`.`event-docs-id` WHERE `sesa-status` != 0";
...
<div class="docs-list">
  ...
  while ($doc = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    ...
    <div class="doc" id="doc<?php echo $i ?>">
      ...
      <div class="docs-list-4">
        <div onclick="acceptDicline(<?php echo $i?>, 'sesa', 'accepted', <?php echo $doc['teamid']?>);">
          <span>Accept</span>
        </div>
        <div onclick="acceptDicline(<?php echo $i?>, 'sesa', 'declined', <?php echo $doc['teamid']?>);">
          <span>Decline</span>
        </div>
      </div>
    </div>
