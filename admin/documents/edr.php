...
$query = "SELECT * FROM `event-docs` JOIN `event-docs-approval` ON `event-docs`.id = `event-docs-approval`.`event-docs-id` WHERE `edr-status` != 0";
...
<div class="docs-list">
  ...
  while ($doc = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    ...
    <div class="doc" id="doc<?php echo $i ?>">
      ...
      <div class="docs-list-2">
        <span>
          <a href="../../deadline-files/<?php echo $doc['edr']; ?>"><?php echo $doc['edr']; ?></a>
        </span>
      </div>
      <div class="docs-list-3" id="docstatus<?php echo $i ?>">
        <span><?php echo getStatus($doc['edr-status']); ?></span>
      </div>
      <div class="docs-list-4">
        <div onclick="acceptDicline(<?php echo $i?>, 'edr', 'accepted', <?php echo $doc['teamid']?>);">
          <span>Accept</span>
        </div>
        <div onclick="acceptDicline(<?php echo $i?>, 'edr', 'declined', <?php echo $doc['teamid']?>);">
          <span>Decline</span>
        </div>
      </div>
    </div>
